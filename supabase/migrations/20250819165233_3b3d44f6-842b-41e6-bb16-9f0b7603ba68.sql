-- Add geocoding capability when properties are created
-- First, ensure properties table has proper latitude and longitude storage

-- Update existing properties without coordinates to have them geocoded
-- This will be done via a trigger that geocodes on insert/update

-- Create a function to geocode addresses (simplified - in production you'd use a real geocoding service)
...
-- Create trigger to geocode addresses on insert and update
